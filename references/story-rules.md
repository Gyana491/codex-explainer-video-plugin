...
- Establish one original top-level `theme_bible` for all panels: named recurring character or object designs, chosen background treatment, illustration style, accent colors, overlay typography, spacing, container/decoration language, icon style, and shadow treatment. Repeat the character and theme anchors in every slide description.
- Make every final composited slide presentation-complete and understandable without narration: one `clear_idea`, a concise editorial overlay title, only the labels and short annotations needed for that idea, at least one named recurring story character or object, and a visual explanation using the most appropriate arrows, paths, diagrams, process flows, charts, text, and objects.
